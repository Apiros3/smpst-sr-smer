...
Lemma _3_19_3_helper : forall G G' p q s l L1 L2 LS LT LS' LT' T,
      ...
      s <> p ->
      projectionC G s T -> 
      (forall pt, isgPartsC pt G -> exists T, projectionC G pt T) ->
      exists T', projectionC G' s T' /\ isMergable T' T. 
Proof.
  intros.
...
Proof.
  ...
  - inversion Ha. subst.
    ...
    destruct H10 as (H10,(Hsa,Hsb)). clear H10 Hsa H9.
    pinversion H4; try easy. subst. clear H12 H13 H16 H17 H18 H21.
    assert(List.Forall (fun u => u = None \/ (exists s' g, u = Some (s', g) /\ forall G' T, 
        gttstepC g G' p q l -> projectionC g s T -> exists T', projectionC G' s T' /\ subtypeC T T' /\ (wfC T -> wfC T'))) ys).
    {
      apply Forall_forall; intros. destruct x.
      - right. specialize(in_some_implies_onth p0 ys H9); intros. destruct H10 as (n, H10).
        destruct p0. exists s0. exists g. split. easy.
       ...
       (exists (s : sort) (g : gtth),
             ...
             (fun u0 : option gtt =>
              ...
              (exists (g0 : gtt) (lsg : list (option (sort * gtt))),
                 ...
                 (exists (s'0 : sort) (Gjk : gtt) (Tp Tq : ltt),
                    ...
                    projectionC Gjk p Tp /\ projectionC Gjk q Tq))) ctxG ->
           wfgC G ->
           exists T' : ltt, projectionC G' s0 T' /\ subtypeC T T' /\ (wfC T -> wfC T')))) xs); intros.
        destruct H17. specialize(H17 H). clear H18 H.
        specialize(H17 (Some (s0, g')) H16). destruct H17; try easy. 
        specialize(Forall_forall (fun u : option (sort * gtt) =>
         u = None \/
        ...
        specialize(Htb p q l g1 G' LP' LQ').
        destruct T1. destruct T1'. 
        specialize(Htb s0 s1 l0 l1 s T0 ctxG). apply Htb; try easy.
        specialize(ishParts_n Hb H13); intros. apply H. easy.
        specialize(ishParts_n Hc H13); intros. apply H. easy.
        specialize(wfgC_after_step (gtt_send s s' ys) g1 s s' n); intros. apply H; try easy.
        pfold. apply steq with (s := s2); try easy. 
    ...
    specialize(_3_19_3_helper_h3 ys ys2 p q l s H35 H9); intros. clear H9 H35.
    
    pinversion H7. subst.
    - exists ltt_end. split.
      pfold. apply proj_end. intros. apply H9.
      specialize(part_after_step (gtt_send s s' ys) (gtt_send s s' ys2) p q r l L1 L2); intros.
      ...
      Forall2
        (fun u v : option (sort * ltt) =>
         ...
         (exists (s : sort) (t t' : ltt),
            u = Some (s, t) /\ v = Some (s, t') /\ isMergable t' t)) ys3 x -> 
      isMergable (ltt_send s' x) (ltt_send s' ys3).
Proof.
  induction x; intros. destruct ys3; try easy. constructor.
  destruct ys3; try easy.
  inversion H. subst. specialize(IHx ys3 s' H5).
  clear H H5. 
  destruct H3.
  - destruct H. subst.
    inversion IHx. subst. constructor.
  - destruct H. destruct H. destruct H. destruct H. destruct H0. subst.
...
Lemma _3_19_3_helper : forall G G' p q s l L1 L2 LS LT LS' LT' T pt,
      wfgC G ->
      projectionC G p (ltt_send q L1) ->
      onth l L1 = Some (LS, LT) ->
      projectionC G q (ltt_recv p L2) ->
      ...
      s <> p ->
      projectionC G s T -> 
      (isgPartsC pt G -> exists T, projectionC G pt T) ->
      exists T', projectionC G' s T' /\ isMergable T' T.
Proof.
  intros. rename H8 into Ht.
  specialize(_a_29_s G p q L1 L2 LS LT LS' LT' l H H0 H1 H2 H3); intros.
  ...
  revert H0 H1 H2 H3 H4 H5 H6 H7 H8 H9 H10 H H13 Ht. revert p q s l G G' L1 L2 LS LS' LT LT' T ctxG.
  induction Gl using gtth_ind_ref; intros.
  - inversion H8. subst.
    ...
    - subst.
      ...
      exists x3. split; try easy. rename x3 into T'.
      specialize(merge_inv_s_after l ys T' T); intros. 
      apply H18; try easy.
    apply proj_mon.
    apply step_mon.
  - rename s into r. rename p into s. rename q into s'. rename p0 into p. rename q0 into q.
    ...
    assert(List.Forall2 (fun u v => (u = None /\ v = None) \/ (exists s' g g', u = Some(s', g) /\ v = Some(s', g') /\ gttstepC g g' p q l /\ forall T, projectionC g r T -> exists T' : ltt,
             projectionC g' r T' /\ isMergable T' T)) ys ys2).
    {
      ...
      revert H H5 H6 H9 H10 H13 H19 H15 H40 H12 Ht.
    ...
    - subst. 
      ...
      apply _3_19_3_helper_h3; try easy. admit.
